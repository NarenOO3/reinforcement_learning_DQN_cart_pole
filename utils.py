...
def get_experiences(memory_buffer):
    ...
    terminated_vals = tf.convert_to_tensor(
        np.array([e.terminated_vals for e in experiences if e is not None]).astype(np.uint8),
        dtype=tf.float32,
    )
...
def get_action(q_values, epsilon=0.0):
    ...
    Args:
        q_values (tf.Tensor):
            The Q values returned by the Q-Network. For the Cart Pole environment
    ...
    Returns:
       ...
       represented by integers in the closed interval [0,1].
    """
